 ...
 */
namespace BGV {
using V = typename params::poly_t::value_type;
using S = typename params::poly_t::signed_value_type;
namespace util {
...
inline void reduce(params::poly_t &coefficients, V multiplier,
		V const &mod_init, V const &mod_initDiv2);
//inline void lift(std::array<mpz_t, params::polyZ_p::degree> &coefficients,params::polyZ_p const &c);
...
public:
	...
	/// Constructors
	ciphertext_t() : c0(0), c1(0), r(0), e1(0), e2(0), pk(nullptr), isnull(true) {}
	ciphertext_t(ciphertext_t const &ct) : c0(ct.c0), c1(ct.c1), r(ct.r), e1(ct.e1), e2(ct.e2) {
		if (ct.pk != nullptr) {
			pk = ct.pk;
		}
		isnull = ct.isnull;
	...
	template <typename T>
	ciphertext_t(T const &value) : c0(0), c1(0), r(0), e1(0), e2(0), pk(nullptr), isnull(true) {
		assert(value == 0);
	}
	template <typename T>
	ciphertext_t(pk_t &pk_in, T const &m) : c1(0), pk(&pk_in), r(0), e1(0), e2(0) {
		if (m == 0) {
			c0 = 0;
			isnull = true;
		} else {
...
void decrypt_poly(params::poly_t &poly_m,
	...
	util::reduce(poly_m, params::plaintextModulus,
			params::poly_t::get_modulus(0), pk.qDivBy2);
...
double noise(M const &message, SK const &sk,PK const &pk, C const &ct) {
	using P = params::poly_t;
...
//	cout << numerator << endl;
	...
	P noise_term = pk.e * ct.r + ct.e1 - sk.value * ct.e2;
//	noise_term = noise_term * t + poly_m;
	noise_term.invntt_pow_invphi();

//	cout << noise_term << endl;
...
//	cout << "}\n{";
	for (auto &v: noise_term) {
		util::center(tmp, v, P::get_modulus(0), pk.qDivBy2);
//		cout << tmp << ",";
		logMax2 = std::max(logMax2, std::log2(std::abs(tmp)));
		tmpMax2 = std::max(tmpMax2, std::abs(tmp));
	}
//	cout << "}\n";
	//cout << "union bound:" << logMax2 << ", " << tmpMax2 << endl;
	//cout << tmpMax << endl;
	return logMax;
}
}  // namespace BGV
...
namespace BGV {
using V = params::poly_t::value_type;
using S = typename params::poly_t::signed_value_type;
namespace util {
