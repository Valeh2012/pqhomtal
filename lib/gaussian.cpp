...
static __m128d V_SIGMA_INV = {0, 0};
static __m128d V_DISCRETE_NORMALISATION = {0, 0};
 ...
 * EndIf
 */
int64_t discrete_gaussian(const double sigma)
{
	...
	V_SIGMA_INV = _mm_set_sd(SIGMA_INV);
	V_DISCRETE_NORMALISATION = _mm_set_sd(DISCRETE_NORMALISATION);

	uint64_t b, i, dcmp, dcmp1, drej, drc, dcmprc;
	
	__m128d vc, vcr, vy, vyr, vyrc, vout; 
	__m128i vb, vcmprc; 
